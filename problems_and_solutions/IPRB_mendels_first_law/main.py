...
FILEPATH = r"IPRB_mendels_first_law\data.txt"

def main():
    with open(FILEPATH) as file:
        homo_dom, hetero, homo_rec = map(int, file.read().strip().split())
    
    total = homo_dom + hetero + homo_rec
    pairs = total * (total - 1) / 2

    p_recessive = (
        (hetero * (hetero - 1) // 2) * 0.25 +
        (homo_rec * (homo_rec - 1) // 2) +
        (hetero * homo_rec * 0.5)
    ) / pairs

    print(f"{(1 - p_recessive):.5f}")
