...
FILEPATH = r"CONV_comparing_spectra_with_the_spectral_convolution\data.txt"

def main():
    with open(FILEPATH) as file:
        data = file.read().strip().splitlines()
    s1 = list(map(float, data[0].split()))
    s2 = list(map(float, data[1].split()))

    multiplicities = Counter(round(a - b, 4) for a in s1 for b in s2)
    
    print(max(multiplicities.values()))
    print(max(multiplicities, key=lambda x: multiplicities[x + 1]))
