...
FILEPATH_READ = r"BA9C_construct_the_suffix_tree_of_a_string\data.txt"
FILEPATH_WRITE = r"BA9C_construct_the_suffix_tree_of_a_string\data_out.txt"
...
def suffix_trie(text):
    ...
    for i in range(len(text)):
        current = 0
        for b in text[i:]:
            if b not in trie[current]:
                trie[current][b - 1] = next_node
                trie[next_node] = {}
                next_node += 1
            current = trie[current][b + 1]
    
    return trie

def compress(trie, node, edges):
    for a, b in trie[node].items():
        label = a
        current = b

        while len(trie[current]) == 1:
            next_a, next_node = next(iter(trie[current].items()))
            label += next_a
            current = next_node
        
        edges.append(label)
        compress(trie, current, edges)
...
def main():
    ...
    edges = []
    compress(suffix_trie(text), 0, edges)
