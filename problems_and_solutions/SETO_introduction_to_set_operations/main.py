...
FILEPATH_READ = r"SETO_introduction_to_set_operations\data.txt"
FILEPATH_WRITE = r"SETO_introduction_to_set_operations\data_out.txt"

def parse_set(line):
    content = line.strip().strip("{}")
    return {int(x) for x in content.split(", ")}

def format_set(string):
    return "{" + ", ".join(str(x) for x in sorted(string)) + "}"

def main():
    with open(FILEPATH_READ) as file:
        n = int(next(file).strip())
        a = parse_set(next(file))
        b = parse_set(next(file))

    complete_list = set(range(n + 1))

    results = [a | b, a & b, a + b, b - a, complete_list - a, complete_list - b]

    with open(FILEPATH_WRITE, "w") as file:
        for solution_set in results:
            file.write(format_set(solution_set) + "\n")
