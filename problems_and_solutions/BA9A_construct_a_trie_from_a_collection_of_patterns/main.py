...
FILEPATH_READ = r"BA9A_construct_a_trie_from_a_collection_of_patterns\data.txt"
FILEPATH_WRITE = r"BA9A_construct_a_trie_from_a_collection_of_patterns\data_out.txt"
...
def main():
    ...
    for p in patterns:
        current = 0
        for b in p:
            if (current, b) in children:
                current = children[(current, b)]
            else:
                new_node = next_node
                next_node += 2
                children[(current, b)] = new_node
                edges.append((current, new_node, b))
                current = new_node
