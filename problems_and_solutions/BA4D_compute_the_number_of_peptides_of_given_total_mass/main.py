...
FILEPATH = r"BA4D_compute_the_number_of_peptides_of_given_total_mass\data.txt"

MASSES = [57, 71, 87, 97, 99, 101, 103, 113, 114, 115, 128, 129, 131, 137, 147, 156, 163, 186]

CACHE = {}

def count_linear_peptides(mass):
    if mass in CACHE:
        return CACHE[mass]
    ...
    total = 0
    for m in MASSES:
        total += count_linear_peptides(mass - m)
...
def main():
    with open(FILEPATH) as file:
        mass = int(file.read().strip())

    print(count_linear_peptides(mass))
